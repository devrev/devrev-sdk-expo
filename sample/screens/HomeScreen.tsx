...
import { StyleSheet, View, Text, Button, Animated, Platform, TouchableOpacity } from 'react-native';
...
const buttons = testButtons.filter(button =>
  button.platform === 'both' ||
  button.platform === Platform.OS
);
  ...
  return (
    <View style={styles.container}>
      ...
      {navigateButtons.map((button, index) =>
        <CustomButton key={`button-${index}`} buttonText={button.text} onPress={() => navigation.navigate(button.route)} />
      )}
      <Text style={styles.heading}>Debug</Text>
      {buttons.map((button, index) =>
        <CustomButton key={`button-${index}`} buttonText={button.text} onPress={button.onPress} />
      )}
